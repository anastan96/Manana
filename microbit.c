...
struct {
    ...
    and in init() for every button a different function is given and when the button is clicked, it will know
...
//The role of this method is to save the image created by the LED's
void save_leds() {
    int x = 0;
    int y = 2;
    int i, j;
...
void print_toolbar() {
    ...
    //It colors the first two rows with blue and prints the title in the middle
    attron(COLOR_PAIR(3));
    for(i = 0; i < col; ++i)
        mvaddch(0, i, ' ');
...
int main() {
    ...
    //keeps the program running continuosly 
    while(!exit) {
        if(redraw) {
            print_toolbar();
            print();
    ...
    //----
        ...
        input = getch(); //It will wait for the user to press a key and then returns the corresponding integer
